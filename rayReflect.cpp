...
int main() {
    ...
    const Vec3 ambient(0, 0, 0);
    const Vec3 blue(0, 0, 1);
    const Vec3 yellow(1, 1, 0);
    const Vec3 dark(.1* 1.4, .1* 1.4, .1* 1.4);
    
    
    
    
    const Sphere sphere(Vec3(0, .3, 0) , .2);
    
    const Triangle triangle(Vec3(0,-.5,.5), Vec3(1, .5, 0), Vec3(0, -.5, -.5));
    const Triangle triangle2(Vec3(0,-.5,.5), Vec3(0, -.5, -.5), Vec3(-1, .5, 0));
    
    
    const Sphere light(Vec3(0, 1, 0), 1);
    
    std::ofstream outR("outR.ppm");
    outR << "P3\n" << W << ' ' << H << ' ' << "255\n";
    ...
    Vec3 CameraLookFrom = Vec3 (0, 0, 1.2);
    ...
    for (int y = 0; y < H; ++y) {
        for (int x = 0; x < W; ++x) {
            ...
            if (sphere.intersect(ray, t)) {
                ...
                double NdotRD = (dot(N, ray.d));
                Vec3 ferR = ray.d - (N * NdotRD * 2);
               Ray refRay(pi, ferR);
                                     
               if (triangle.intersect(refRay, tTriangle))
                 {
                     const Vec3 pi2 = refRay.o + refRay.d*tTriangle;
                     double bias = .0001;
                     Ray shadowRay((pi2 + N * bias), (L).normalize());
             ...
             else if(triangle2.intersect(refRay, tTriangle2))
                 {
                     const Vec3 pi2 = refRay.o + refRay.d*tTriangle;
                     double bias = .0001;
                     Ray shadowRay((pi2 + N * bias), (L).normalize());
            ...
            //    pix_col = dark * 1.4; // * ambient ; //(ambient + diffuse * fmax(0.0, dot(N,L))); //+ specHigh * white * pow(fmax(0.0, dot(v,r)), phong);
